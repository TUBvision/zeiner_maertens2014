...
svg('figs/reflectance.svg', w=18,h=5, pointsize=8)
...
for (obs in observers)
    ...
    single.fit  = summary(lm(single_data$match ~ single_data$x_reflect))
    global_rsq[obs,] = single.fit$r.squared

	x_values=as.numeric(row.names(agg.m.matches))
	
	## PLAIN data points
	plot(x_values,  agg.m.matches[,'plain'],  pch=21, bg='white', col='black', xlab='povray  reflectance', ylab='', yaxt='n', xlim=c(0,2.3), ylim=c(0,420))
	...
	for (curr_med in colnames(agg.m.matches))
		{
		curr.fit = lm(agg.m.matches[,curr_med] ~ x_values)
		...
		abline(curr.fit, col= unlist(all_cols[curr_med]), lwd=1)
		arrows(x_values, agg.m.matches[,curr_med]-agg.sem.matches[,curr_med], x_values, agg.m.matches[,curr_med]+agg.sem.matches[,curr_med], length=0.01, angle=90, code=3)

		points(x_values, agg.m.matches[,curr_med], pch=21, bg=unlist(all_cols[curr_med]), col='black')
		}
...
write.table(format(global_rsq, digits=4), 'stat_out/reflectance_global_rsq.txt', quote=FALSE)
write.table(format(slopes, digits=4),     'stat_out/reflectance_slopes.txt',     quote=FALSE)
write.table(format(intercepts, digits=4), 'stat_out/reflectance_intercepts.txt', quote=FALSE)
write.table(format(rsq, digits=4),        'stat_out/reflectance_rsq.txt',        quote=FALSE)
